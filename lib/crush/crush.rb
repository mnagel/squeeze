...
#      # TODO wait a second...
  ...
  def crush_mouse_down
    $engine.thing_not_to_intersect.each do |t|
      distance = (t.pos - $engine.mouse.model.pos).abs(false)
      distance += Settings.mousedef

      distance /= Settings.mousedef
      factor = 3
      distance /= factor
      puts distance
      t.v += (t.pos - $engine.mouse.model.pos).unit * (mouse.model.size.x / Settings.mousedef / distance)
...
begin
  require 'glcrush' # TODO do not have constant here
  puts Settings.infotext
  $engine = CrushGameEngine.new
